...
export default function GridCellConfig({ data, }) {
    ...
    return (<Form labelCol={labelCol}>
      <Form.Item label={"Column"}>
        <>
          {data &&
        data.lanes &&
        data.lanes.map((lane, index) => (<LaneConfig key={index} index={index} data={lane} onResize={(span) => {
            const finalSpan = typeof span === "string" || !span ? 0 : span;
            designerDispatch({
                type: "UPDATE",
                data: {
                    ...data,
