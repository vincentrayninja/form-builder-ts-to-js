import { set, validateFormat, validateRequired, setControlValues, } from "../util";
 ...
 * @param handler
 */
export function cloneAndForEach(root, handler) {
 ...
 * @param handler
 */
export function forEach(root, handler) {
...
export function filter(root, predicate) {
    const result = [];
    forEach(root, (value) => {
...
export function locate(root, matchFunc) {
    let location = null;
...
export function deleteActive(rootCellData) {
    const location = locate(rootCellData, (item) => item.active !== undefined && item.active);
    if (location) {
        const [cellLocation, list] = location;
        list.splice(cellLocation.index, 1);
    }
}
export function getCellDataList(root, parentId, index) {
    let list = null;
    const func = function (data) {
        if (data.id === parentId) {
...
function drop(root, cell, dropItemId, position) {
    const [dropLocation, dropList] = locate(root, (item) => item.id === dropItemId);
    if (position === "up") {
...
export function reducer(state, action) {
    ...
    if (action.type === "POSITIONED_MOVE") {
        const [dragLocation, dragList, dragCell] = locate(copy, (item) => item.id === action.id);
        ...
        const cells = getCellDataList(copy, action.location.parentId, action.location.index);
        cells.push(action.dragItem);
        active(copy, action.dragItem.id);
    }
    else if (action.type === "UPDATE") {
        const [location, list] = locate(copy, (data) => data.id === (action.id || action.data.id));
        list.splice(location.index, 1, action.data);
    }
    else if (action.type === "MOVE") {
        const [location, list, cell] = locate(copy, (item) => item.id === action.id);
        list.splice(location.index, 1);
        const cellDataList = getCellDataList(copy, action.location.parentId, action.location.index);
...
export function getActive(root) {
    const location = locate(root, (item) => item.active !== undefined && item.active);
...
export function createWidgetInstance(item, customCells) {
    if (item.createWidgetInstance) {
        return item.createWidgetInstance();
    }
    else if (customCells) {
        const find = customCells.find((customCell) => customCell.type === item.type);
