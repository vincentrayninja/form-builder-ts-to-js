...
`;
export const Designer = forwardRef(({ customCells, availableCustomCells, toolbar = true, defaultCellData, builtinCellDataTypes, onChange, style, }, ref) => {
    const { control, register, handleSubmit, watch, formState: { errors }, } = useForm();
    const onSubmit = (data) => console.log("data", data);
    const [data, designerDispatch] = useReducer(reducer, defaultCellData || rootCellData);
    ...
    const widgetGroups = useMemo(() => {
        ...
        return WidgetGroups.map((group) => ({
            name: group.name,
            widgets: group.widgets.filter((widget) => builtinCellDataTypes.indexOf(widget.type) > -1),
    ...
    return (<>
          ...
          <InteractContext.Provider value={interactions}>
            <DndProvider backend={HTML5Backend}>
              <FullHeightBorderedLayout style={style}>
                <LeftSider>
                  {widgetGroups.map((g) => (<WidgetGroup key={g.name} name={g.name} list={g.widgets}/>))}
                  {finalAvailableCells && (<WidgetGroup key={"Custom"} name={"Custom"} list={[
        ...
        console.log("Save Form---->", JSON.stringify(data));
    }}>
                            Save
                          </Button>
                        </Space>
                    ...
                    <WhiteContent style={contentStyle}>
                      <DnDCell className={"root"} cellData={data} index={0} customCells={customCells}/>
                    </WhiteContent>
                  </WhiteLayout>
                </WhiteContent>
                <WhiteSider width={280} style={rightSiderStyle}>
                  {active ? (active.type === "grid" ? (<GridCellConfig data={active}/>) : active.type === "section" ? (<SectionCellConfig data={active}/>) : active.type === "address" ? (<AddressCellConfig data={active}/>) : active.type === "tab" ? (<TabCellConfig data={active}/>) : active.type === "pages" ? (<PageCellConfig data={active}/>) : active.type === "datetime" ? (<DateCellConfig data={active}/>) : active.type === "select" ? (<SelectCellConfig data={active}/>) : active.type === "gender" ? (<GenderCellConfig data={active}/>) : active.type === "checkbox" ? (<CheckboxCellConfig data={active}/>) : active.type === "label" ? (<LabelCellConfig data={active}/>) : active.type === "input" ? (<InputCellConfig data={active}/>) : ((customCells &&
        customCells.some((item) => item.type === active.type) &&
        customCells.filter((item) => item.type === active.type)[0].config &&
        React.createElement(customCells.filter((item) => item.type === active.type)[0].config, {
            data: active,
        ...
        <Modal width={1000} title={"Preview"} visible={previewDialogVisible} onCancel={() => setPreviewDialogVisible(false)} footer={[
        ...
        <Button key={"validate"} onClick={() => previewRef.current.validate()}>
              Validate
            </Button>,
    ]}>
          {previewData && (<form onSubmit={handleSubmit(onSubmit)}>
              <Form control={control} customCells={customCells} ref={previewRef} data={cloneAndForEach(previewData, (data) => {
